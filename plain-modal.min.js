  ...
  function PlainModal(content, options) {
    ...
    if (!document.getElementById(STYLE_ELEMENT_ID)) {
      ...
      window.addEventListener('keydown', function (event) {
        var key = void 0,
            topProps = void 0;
        if (closeByEscKey && ((key = event.key.toLowerCase()) === 'escape' || key === 'esc') && (topProps = shownProps.length && shownProps[shownProps.length - 1]) && (traceLog.push('<keydown/>', 'CLOSE', '_id:' + topProps._id),(console.log('CTIME: '+(Date.now()-window.ctime))), true) && // [DEBUG/]
        _close(topProps)) {
...
function finishHiding(props, sync) {
  ...
  traceLog.push('sync:' + !!sync); // [DEBUG/]
    traceLog.push('[[ADD-STYLE_CLASS_HIDE]]'); // [DEBUG/]
...
function _show(props, force) {
  ...
  if (props.state === STATE_HIDDEN) {
    ...
    traceLog.push('[[REMOVE-STYLE_CLASS_HIDE]]'); // [DEBUG/]
...
function _hide(props, force, sync) {
  ...
  elmOverlayClassList.toggle(STYLE_CLASS_FORCE, !!force);
  elmOverlayClassList.remove(STYLE_CLASS_SHOW);//xxx
    props.elmOverlay.offsetWidth; /* force reflow */ // eslint-disable-line no-unused-expressions
...
var PlainOverlay = function () {
  ...
  function PlainOverlay(target, options) {
    ...
    (function (listener) {
      ...
      console.log('OP: ' + (event.target === elmOverlay) +', '+ (event.propertyName === 'opacity') + ' state: '+props.state);
      traceLog.push('OP: ' + (event.target === elmOverlay) +', '+event.propertyName+' '+ (event.propertyName === 'opacity') + ' state: '+STATE_TEXT[props.state]); // [DEBUG/]
    ...
    (function (listener) {
      ['transitionstart', 'webkitTransitionStart', 'oTransitionStart', 'otransitionstart'].forEach(function (type) {
        elmOverlay.addEventListener(type, listener, true);
      });
    })(function (event) {
      console.log('TransitionStart: ' + (event.target === elmOverlay) +', '+ event.propertyName+' '+ (event.propertyName === 'opacity') + ' elapsedTime: '+event.elapsedTime + ' state: '+props.state);
      traceLog.push('TransitionStart: ' + (event.target === elmOverlay) +', '+event.propertyName+' '+ (event.propertyName === 'opacity') + ' elapsedTime: '+event.elapsedTime + ' state: '+STATE_TEXT[props.state]); // [DEBUG/]
    });
