  ...
  if (props.parentProps) {
    ...
    */
    ...
    if (parentProps.state === STATE_OPENED) {
      elmOverlay.style[CSSPrefix.getName('transitionDuration')] = props.options.duration === DURATION ? '' : "".concat(props.options.duration, "ms"); // [DEBUG]
...
function _open(props, force) {
  ...
  */
  ...
  if (props.state === STATE_CLOSED) {
    ...
    mClassList(props.elmOverlay).add(STYLE_CLASS_OVERLAY_FORCE).remove(STYLE_CLASS_OVERLAY_HIDE); // [DEBUG]

    traceLog.push("elmOverlay.CLASS_FORCE:".concat(mClassList(props.elmOverlay).contains(STYLE_CLASS_OVERLAY_FORCE)));
    traceLog.push("elmOverlay.CLASS_HIDE:".concat(mClassList(props.elmOverlay).contains(STYLE_CLASS_OVERLAY_HIDE))); // [/DEBUG]
  }
...
window.PlainOverlay = PlainOverlay; // [/DEBUG]
